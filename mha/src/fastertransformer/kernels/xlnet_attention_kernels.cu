                                      ...
                                      const int voff2)
{
    ...
    if (seq2 < seq_len) {
        ...
        out_index = batch * seq_len + seq2;
        mask = attn_mask[out_index] == 0 ? large_value : 0;
        score = score + mask;
    } 
    ...
    __syncthreads();
    if (seq2 < seq_len) {
        attn_score[index] = attn_mask[out_index] == 0 ? 0 : (T)(qk_tmp / s_sum);
    }
    // end softmax
...
transpose102_v2(T* dst, const T* src, const int off0, const int i_off1, const int o_off1, const int off2)
{
    ...
    T tmp;
    if (x[3] < off2) {
        int input_index = x[0] * off0 + x[2] * o_off1 + x[1] * off2 + x[3];  // [batch, 0, 1, 2]=[d0,d1,d2,d3]
        tmp = src[input_index];
        int out_index = x[0] * off0 + x[1] * i_off1 + x[2] * off2 + x[3];  // [batch, 1, 0, 2]=[d0,d2,d1,d3]
    ...
    int batch_size, int seq_len, int head_num, int size_per_head, T* attn_vec_trans, T* attn_vec, cudaStream_t stream)
{
    ...
    transpose102<<<grid_trans_v, block_trans_v, 0, stream>>>(attn_vec_trans, attn_vec, off0, i_off1, o_off1, off2);
}    
/*
...
template void invokeTranspose102<float>(
    int batch_size, int seq_len, int head_num, int size_per_head, float* attn_vec_trans, float* attn_vec, cudaStream_t stream);

template void invokeTranspose102<half>(
    int batch_size, int seq_len, int head_num, int size_per_head, half* attn_vec_trans, half* attn_vec, cudaStream_t stream);
